...
| # | Issue | Category |
|---|-------|----------|
| — | **E2E test suite completion:** Trunk test `sync-inject-golden-path.spec.ts` (BFA sync → inject → import), branch tests (classification, error handling, decisions, GDocs matching), deep tests (hierarchy creation, formatting). Plan: [e2e-import-injection-loop.md](docs/plans/e2e-import-injection-loop.md). | Testing |
...
| # | Issue | Closed By |
|---|-------|-----------|
| — | **E2E test infrastructure and CSV import golden path (Feb 10 2026):** Playwright test foundation — auth fixtures, test CSV data (small + BFA subset), API shortcut helpers, breadcrumb/waypoint helpers, initial trunk test (`csv-import-golden-path.spec.ts`). Plan doc: [e2e-import-injection-loop.md](docs/plans/e2e-import-injection-loop.md) defines garden path architecture (trunk vs branch vs deep tests), waypoint system for MCP agent consumption, naming conventions. Stack PRs: #485 (gitignore playwright artifacts), #486 (e2e infrastructure). | PRs #485-486 |
