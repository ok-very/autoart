...
**Electron migration (#64):** PyInstaller + MSIX pipeline replaced with Electron + NSIS. AutoHelper's Python FastAPI backend is unchanged — Electron spawns it as a child process and manages its lifecycle (health polling, graceful `POST /shutdown`, SIGKILL fallback). The React dashboard (`apps/autohelper-electron/src/renderer/`) replaces the old Jinja-served HTML dashboard. Distribution uses `python-build-standalone` embedded at build time via `prepare-python.ts`. CI workflow (`build-autohelper-electron.yml`) produces a signed NSIS installer on `autohelper-v*` tags. Old PyInstaller workflow deleted.
