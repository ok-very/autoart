...
export type DrawerSideEffect =
    | { type: 'create'; entityType: 'record' | 'node' | 'definition' | 'link' | 'project' | 'connection' | 'event' }
    ...
    | { type: 'import'; entityType: 'record' }
    | { type: 'export'; entityType: 'project' };
...
export interface DrawerContextMap {
    ...
    'import-workbench': ImportWorkbenchContext;
    'export-workbench': ExportWorkbenchContext;
}
...
/** Context for Monday board selector drawer */
export interface MondayBoardsContext {
    /** Callback when a board is selected for import */
    onBoardImport: (boardId: string, options: { includeSubitems: boolean }) => void;
    /** Whether import is in progress */
    isImporting?: boolean;
}

/** Context for Import Workbench drawer */
export interface ImportWorkbenchContext extends IngestionContext {
    /** Optional callback when import completes successfully */
    onImportComplete?: (result: { recordsCreated: number }) => void;
}

/** Context for Export Workbench drawer */
export interface ExportWorkbenchContext {
    /** Pre-selected project IDs to export */
    projectIds?: string[];
    /** Pre-selected export format */
    format?: 'rtf' | 'markdown' | 'plaintext' | 'csv' | 'google-doc' | 'google-sheets' | 'google-slides';
    /** Optional callback when export completes */
    onExportComplete?: (result: { format: string; url?: string }) => void;
}
