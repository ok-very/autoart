...
class TestIndexService:
    ...
    def test_rebuild_with_specific_root_id(
        ...
        db.execute(
            "INSERT INTO roots (root_id, path, enabled) VALUES (?, ?, 1)", (r1_id, str(root1_path))
        )
        db.execute(
            "INSERT INTO roots (root_id, path, enabled) VALUES (?, ?, 1)", (r2_id, str(root2_path))
        )
        db.commit()
        ...
        # Assert
        files_r1 = db.execute(
            "SELECT count(*) as c FROM files WHERE root_id = ?", (r1_id,)
        ).fetchone()["c"]
        files_r2 = db.execute(
            "SELECT count(*) as c FROM files WHERE root_id = ?", (r2_id,)
        ).fetchone()["c"]
    ...
    def test_rebuild_indexes_files(self, client: TestClient, temp_dir: Path, test_db) -> None:
        ...
        file_entry = db.execute(
            "SELECT * FROM files WHERE canonical_path = ?", (canonical,)
        ).fetchone()

        # Fallback for Windows casing
        if not file_entry:
            file_entry = db.execute(
                "SELECT * FROM files WHERE lower(canonical_path) = lower(?)", (canonical,)
            ).fetchone()
    ...
    def test_rebuild_skips_content_hash_for_large_files_if_not_forced(
        ...
        canonical = str(test_file.resolve())
        row = db.execute(
            "SELECT content_hash FROM files WHERE canonical_path = ?", (canonical,)
        ).fetchone()
        if not row:
            row = db.execute(
                "SELECT content_hash FROM files WHERE lower(canonical_path) = lower(?)",
                (canonical,),
            ).fetchone()
        ...
        row = db.execute(
            """SELECT content_hash FROM files
               WHERE canonical_path = ? OR lower(canonical_path) = lower(?)""",
            (canonical, canonical),
        ).fetchone()
    ...
    def test_rebuild_handles_nested_dirs(self, client: TestClient, temp_dir: Path, test_db) -> None:
        ...
        # Check deep file
        file_entry = db.execute(
            "SELECT * FROM files WHERE rel_path = ?", ("a\\b\\c\\d\\deep.txt",)
        ).fetchone()
    ...
    def test_rescan_removes_deleted_files(
        ...
        row = db.execute(
            "SELECT file_id FROM files WHERE canonical_path = ?", (canonical,)
        ).fetchone()
        if not row:
            row = db.execute(
                "SELECT file_id FROM files WHERE lower(canonical_path) = lower(?)", (canonical,)
            ).fetchone()
        assert row is not None
...
class TestIndexEndpoints:
    ...
    def test_rebuild_endpoint(self, client: TestClient, temp_dir: Path) -> None:
    ...
    def test_roots_endpoint(self, client: TestClient, temp_dir: Path) -> None:
