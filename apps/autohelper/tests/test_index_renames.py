...
def get_file_id(db, path: str) -> str | None:
    ...
    if not row:
        row = db.execute(
            "SELECT file_id FROM files WHERE lower(canonical_path) = lower(?)", (path,)
        ).fetchone()
...
class TestRenameDetection:
    ...
    def test_rename_preserves_file_id_and_creates_alias(
        ...
        row_new = db.execute(
            "SELECT file_id, canonical_path FROM files WHERE file_id = ?", (file_id,)
        ).fetchone()
        ...
        # C. Ref updated
        row_ref = db.execute(
            "SELECT canonical_path FROM refs WHERE ref_id = ?", ("ref1",)
        ).fetchone()
    ...
    def test_unreferenced_file_is_treated_as_new(
        ...
        canonical_new = str(new_file.resolve())
        row_new = db.execute(
            "SELECT file_id FROM files WHERE canonical_path = ?", (canonical_new,)
        ).fetchone()
        if not row_new:
            row_new = db.execute(
                "SELECT file_id FROM files WHERE lower(canonical_path) = lower(?)", (canonical_new,)
            ).fetchone()
        assert row_new is not None
        ...
        # No alias created
        row_alias = db.execute(
            "SELECT * FROM file_aliases WHERE old_canonical_path = ?", (canonical_old,)
        ).fetchone()
