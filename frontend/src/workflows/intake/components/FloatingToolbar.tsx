...
export function FloatingToolbar({ activeBlockId, onAddBlock }: FloatingToolbarProps) {
    ...
    const renderSection = (title: string, tools: typeof inputTools) => (
        <div className="mb-2">
            <p className="text-[10px] text-ws-muted uppercase tracking-wide mb-1 px-1">{title}</p>
    ...
    return (
        <div className="absolute right-[-180px] top-0 w-[160px] bg-ws-panel-bg p-2 rounded-lg shadow-lg border border-ws-panel-border max-h-[70vh] overflow-y-auto">
            {renderSection('Input', inputTools)}
            <hr className="my-2 border-ws-panel-border" />
