...
### Why pnpm?
- **Disk efficient**: Uses a content-addressable store, packages are linked not copied
- **Fast**: Parallel installation, cached packages
- **Strict**: Prevents phantom dependencies (can only import what's declared)
- **Workspace support**: Native monorepo support with `workspace:*` protocol
