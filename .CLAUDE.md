...
### Browser Subagent
- **NEVER** use complex multi-line strings or git commands in CSS selectors or data attributes
- **AVOID** selectors with embedded quotes, newlines, or special characters
- **DO** use simple, clean selectors: `[data-tooltip-id="simple-id"]`
- **DON'T** use: `[data-tooltip-id="gh pr create --title \"...\""]` ❌

### Command Line Operations
- **Git commits**: Use simple, short commit messages in one line
  ```powershell
  # ✅ Correct
  git commit -m "chore: remove Docker references"
  
  # ❌ Wrong - multi-line breaks selector logic
  git commit -m "title
  
  body with multiple lines"
  ```

- **Escaping quotes**: In PowerShell, use single quotes for outer string, double quotes inside
  ```powershell
  # ✅ Correct
...
1. **Keep strings simple**: No embedded newlines, quotes, or complex formatting
2. **Use single quotes** for PowerShell commands when possible
3. **Avoid HTML/CSS special chars** in any identifier or selector
4. **Test command syntax** before using in browser automation
5. **Split complex operations** into multiple simple tool calls
