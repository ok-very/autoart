...
export const DRAWER_DEFINITIONS: Record<keyof DrawerContextMap, DrawerDefinition> = {
    ...
    'monday-boards': {
        id: 'monday-boards',
        title: 'Select Monday Board',
        size: 'xl',
        sideEffects: [{ type: 'select', entityType: 'record' }],
        dismissible: true,
        showClose: true,
    },
    'import-workbench': {
    ...
    'export-workbench': {
        ...
        size: 'full',
        sideEffects: [], // Export is read-only
        dismissible: true,
        showClose: true,
    },
};
...
const ImportWorkbenchDrawerView = lazy(() =>
    import('../ui/drawer/views/ImportWorkbenchDrawerView').then((m) => ({ default: m.ImportWorkbenchDrawerView }))
);
const ExportWorkbenchDrawerView = lazy(() =>
    import('../ui/drawer/views/ExportWorkbenchDrawerView').then((m) => ({ default: m.ExportWorkbenchDrawerView }))
);
...
export function DrawerRegistry({ type, context, onClose, onResult }: DrawerRegistryProps) {
    ...
    const handleSubmit = (result: DrawerResult) => {
        if (isDev) {
    ...
    // New contract views use full DrawerProps, legacy views use spread props
    const renderView = () => {
        switch (type) {
            case 'create-record':
            ...
            // New contract views - pass full DrawerProps
            case 'import-workbench':
                return (
                    <ImportWorkbenchDrawerView
