...
class IndexService:
    ...
    def rebuild_index(
        self, specific_root_id: str | None = None, force_hash: bool = False
    ) -> RunResponse:
        """
        ...
        # Since sqlite isolation can be tricky, we can use a conditional
        # insert or check first in transaction.
        # Ideally, we rely on a UNIQUE index on 'running' status, but status
        # changes. So we check for existence of 'running' status in a tx.

        try:
            # Check if any run is currently running
            existing = self.db.execute(
                "SELECT index_run_id FROM index_runs WHERE status = ?", (IndexRunStatus.RUNNING,)
            ).fetchone()
            ...
            if existing:
                ...
                raise ConflictError(
                    message="Index run already in progress", resource_id=existing["index_run_id"]
                )
        ...
        try:
            ...
            if specific_root_id:
                ...
                row = self.db.execute(
                    "SELECT root_id, path FROM roots WHERE root_id = ?", (specific_root_id,)
                ).fetchone()
                if not row:
            ...
            self.db.execute(
                ...
                WHERE index_run_id = ?
                """,
                (IndexRunStatus.COMPLETED, utcnow_iso(), json.dumps(vars(total_stats)), run_id),
            )
            self.db.commit()
            ...
            self.db.execute(
                ...
                WHERE index_run_id = ?
                """,
                (IndexRunStatus.FAILED, utcnow_iso(), json.dumps({"error": str(e)}), run_id),
            )
            self.db.commit()
            raise
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        running = self.db.execute(
            "SELECT * FROM index_runs WHERE status = ?", (IndexRunStatus.RUNNING,)
        ).fetchone()
    ...
    def _sync_configured_roots(self) -> None:
        ...
        # 1. Add new roots
        for path_str, _path_obj in current_config_paths.items():
            # Check if exists (by path)
            row = self.db.execute(
                "SELECT root_id FROM roots WHERE path = ?", (path_str,)
            ).fetchone()
            if not row:
        ...
        try:
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        if existing:
                            ...
                            if (
                                not force_hash
                                and existing["size"] == file_stat.size
                                and existing["mtime_ns"] == file_stat.mtime_ns
                            ):
                                # No change
                                self.db.execute(
            ...
            for rel_path, stat in potential_new_files:
                ...
                if stat.is_offline:
                    ...
                    candidates = [
                        m
                        for m in missing_by_size.get(stat.size, [])
                    ...
                    if candidates:
                        try:
                            ...
                            hash_matches = [
                                c for c in candidates if c["content_hash"] == current_hash
                            ]
                            ...
                            if hash_matches:
                                ...
                                if target:
                                    ...
                                    is_referenced = self.db.execute(
                                        "SELECT 1 FROM refs WHERE file_id = ?", (target["file_id"],)
                                    ).fetchone()
    ...
    def _execute_rename(
        ...
        self.db.execute(
            "UPDATE refs SET canonical_path = ? WHERE file_id = ?", (new_canonical, file_id)
        )
    ...
    def _upsert_file(
        ...
        if existing_id:
            ...
            self.db.execute(
                ...
                WHERE file_id = ?
                """,
                (stat.size, stat.mtime_ns, content_hash, utcnow_iso(), utcnow_iso(), existing_id),
            )
        else:
            # Insert
