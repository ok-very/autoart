    ...
    return (
        <DropdownMenu.Portal>
            <DropdownMenu.Content
                className={clsx(
                    "z-50 min-w-[8rem] overflow-hidden rounded-md border border-slate-200 bg-white p-1 font-sans text-slate-950 shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
                    className
                )}
...
export function DropdownItem({ children, onSelect, className, disabled }: DropdownItemProps) {
    return (
        <DropdownMenu.Item
            ...
            disabled={disabled}
            className={clsx(
                "relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm font-sans outline-none transition-colors focus:bg-slate-100 focus:text-slate-900 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
                className
            )}
        >
            {children}
...
export function DropdownLabel({ children, className }: { children: ReactNode, className?: string }) {
    return (
        <DropdownMenu.Label className={clsx("px-2 py-1.5 text-xs font-sans font-semibold text-slate-500", className)}>
            {children}
        </DropdownMenu.Label>
    );
}
