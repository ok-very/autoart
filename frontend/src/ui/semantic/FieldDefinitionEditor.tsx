...
export function FieldDefinitionEditor({ field }: FieldDefinitionEditorProps) {
        ...
        if (!options.includes(val)) {
            ...
            if (field.type === 'status') {
                setStatusConfig(prev => ({
                    ...prev,
                    [val]: { label: val, colorClass: 'bg-slate-100 text-ws-fg' }
                }));
            }
        }
        setOptionInput('');
    ...
    if (isLoading) return <div className="p-8 text-center text-ws-muted">Loading definition...</div>;
    ...
    return (
        <div className="flex flex-col h-full bg-ws-panel-bg">
            ...
            <div className="h-10 px-4 border-b border-ws-panel-border flex items-center justify-between bg-ws-panel-bg">
                <div>
                    <h2 className="text-ws-h2 font-semibold text-ws-fg">{label || field.label}</h2>
                    <div className="text-xs text-ws-muted flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto p-6">
                <div className="max-w-2xl space-y-8">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-semibold text-ws-fg pb-2 border-b border-ws-panel-border">
                            General Properties
                        </h3>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            />
                            <label htmlFor="required-check" className="text-sm text-ws-text-secondary">
                                Required Field
                            </label>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    onClick={addOption}
                                    type="button"
                                    className="px-3 py-2 bg-slate-100 text-ws-text-secondary rounded-md hover:bg-slate-200"
                                >
                                    <Plus size={18} />
                                </button>
                            </div>
                            ...
                            <div className="space-y-2">
                                {options.map(opt => (
                                    <div key={opt} className="flex items-center gap-3 p-3 bg-ws-bg rounded-lg border border-ws-panel-border">
                                        <div className="flex-1 font-medium text-ws-text-secondary">{opt}</div>

                                        {field.type === 'status' && (
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs text-ws-muted">Color:</span>
                                                <select
                                        ...
                                        <button
                                            ...
                                            className="text-ws-muted hover:text-red-500 transition-colors"
                                        >
                                            <X size={16} />
                                        </button>
                                    </div>
                                ...
                                {options.length === 0 && (
                                    <div className="text-sm text-ws-muted italic text-center py-4">
                                        No options defined
                                    </div>
                                )}
                            </div>
