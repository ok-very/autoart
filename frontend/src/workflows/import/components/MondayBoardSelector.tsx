...
export function MondayBoardSelector({ onImport, isImporting }: MondayBoardSelectorProps) {
    ...
    if (!connections?.monday?.connected && !boards) {
        return (
            <div className="p-6 text-center">
                ...
                <h3 className="font-medium text-ws-fg mb-1">Monday.com not connected</h3>
    ...
    if (isLoading) {
        return (
            <div className="p-6 text-center">
                <Loader2 className="w-8 h-8 mx-auto text-ws-muted animate-spin" />
                <p className="text-sm text-ws-text-secondary mt-2">Loading boards...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="p-6 text-center">
                ...
                <h3 className="font-medium text-ws-fg mb-1">Failed to load boards</h3>
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="p-4 border-b border-ws-panel-border">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 text-sm border border-ws-panel-border rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                    />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto p-4">
                {filteredBoards.length === 0 ? (
                    <div className="text-center py-8 text-ws-text-secondary text-sm">
                        No boards found
                    </div>
                ) : (
                    <div className="space-y-4">
                        {Array.from(boardsByWorkspace.entries()).map(([workspace, workspaceBoards]) => (
                            <div key={workspace}>
                                <div className="flex items-center gap-2 text-xs font-medium text-ws-muted uppercase mb-2">
